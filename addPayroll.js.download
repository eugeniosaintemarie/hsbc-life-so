...
define(["react", "loadsh", "../common/modalReactBootstrap", "../common/inputFile2", "../controller/nominaController", "../common/errorExcel", "../common/buttonLoading"], function (React, Loadsh, ModalReactBootstrap, InputFile2, NominaController, ErrorExcel, ButtonLoading) {
  var AddPayroll = function (_React$Component) {
    ...
    function AddPayroll(props) {
      ...
      _this._handleValidateButton = function () {
        ...
        _this.nominaController.validateNomina(listExcelClon, function (fielProcess) {
          _this.setState({
      ...
      _this._handleSendMailButton = function () {
        ...
        _this.state.listExcel.map(function (item) {
          if (item.VALIDACION != "MAILOK" && item.VALIDACION != "NOEXIST" && item.VALIDACION != "NODESIGNA") listToSend.push(item);
        });
        if (listToSend.length > 0) _this.setState({ enviarLoading: true });
        _this.nominaController.sendNomina(listToSend, _this.props.product, function (errors) {
